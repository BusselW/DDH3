...
const Icons = {
    ...
    ChevronLeft: () => h('svg', {width: 16, height: 16, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round"}, h('polyline', {points: "15 18 9 12 15 6"})),
...
export const AdminMenu = ({ selectedItem, selectedProblem, isAdmin, onRefresh }) => {
    ...
    return h('div', null,
        h('div', { className: `admin-menu ${isExpanded ? 'expanded' : 'collapsed'}` },
            h('div', { className: 'admin-header' }, 
                h('div', { style: { display: 'flex', alignItems: 'center', gap: '10px', flex: 1, overflow: 'hidden' } },
                    h(Icons.Settings), 
                    h('span', { className: 'admin-text' }, 'Beheer Menu')
                ),
                h('button', { 
                ...
                }, isExpanded ? h(Icons.ChevronRight) : h(Icons.ChevronLeft))
            ),
